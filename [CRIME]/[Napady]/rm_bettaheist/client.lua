...
Citizen.CreateThread(function()
    ...
    exports[Config['BettaHeist']['framework']['targetScript']]:AddBoxZone("BettaHeist", Config['BettaHeist']['startHeist']['pos'], 1.1, 2.35, 
    {
        name = "BettaHeist",
        heading = 20.0,
        debugPoly = false,
        ...
        if data['status'] then
            TriggerCallback('bettaheist:server:checkTime', function(data)
                if data['status'] then
                    ...
                    ESX.ShowNotification(Strings['heist_info'])
                    SetupBetta()
                end
            end)
        end
...
AddEventHandler('bettaheist:client:sync', function(type, args)
    ...
    elseif type == 'mainLoop' then
        for k, v in pairs(Config['BettaSetup']['actions']) do
            exports[Config['BettaHeist']['framework']['targetScript']]:AddBoxZone("BettaHeist"..k, v['coords'], v['length'], v['width'], 
            {
                name = "BettaHeist"..k,
                heading = v['heading'],
    ...
    elseif type == 'resetHeist' then
        for k, v in pairs(Config['BettaSetup']['actions']) do
            exports[Config['BettaHeist']['framework']['targetScript']]:RemoveZone("BettaHeist"..k)
        end
        dTrollys = {}
...
function SetupBetta()
    ...
    TriggerServerEvent('bettaheist:server:policeAlert', Config['BettaSetup']['main'])
...
function BoltCutter()
    TriggerCallback('bettaheist:server:hasItem', function(data)
        if data['status'] then
            ...
            ESX.ShowNotification(Strings['need_this'])
        end
