$(document).ready(function () {
                ...
                for (let key in data.Police) {
                    ...
                    $('#onduty-info').append(
                        `<div class='default-duty-info' style='margin-top: 8px;'>
                            <div class='duty-who'>${data.Police[key].firstname + ' ' + data.Police[key].lastname}</div>
                            <div class="duty-number">${data.Police[key].phone_number}</div>
    ...
    $(document).on ("click", "#file-content-removemandat", function() {
        ...
        $.post('http://esx_lspdmdt/RemoveMandatKartoteka', JSON.stringify({id: id, identifier: identifier}));
        $(this).parent().fadeOut(500);
    });
    ...
    $(document).on ("click", "#file-content-removeposzukiwania", function() {
        ...
        $.post('http://esx_lspdmdt/RemovePoszukiwaniaKartoteka', JSON.stringify({identifier: identifier, reason: reason}));
        console.log(reason)
        $(this).parent().fadeOut(500);
    });
    ...
    $(document).on ("click", "#file-content-removenotatki", function() {
        ...
        $.post('http://esx_lspdmdt/RemoveNotatkiKartoteka', JSON.stringify({identifier: identifier, note: note}));
        $(this).parent().fadeOut(500);
    });
    ...
    $(document).on ("click", "#file-more-info", function (event) {
        ...
        $.post('http://esx_lspdmdt/PersonMoreInfo', JSON.stringify({identifier: identifier}), function(moreinfodata){
    ...
    $('#file-button-search').on('click', function(){
        ...
        if(reasonarea != "") {
            ...
            $.post('http://esx_lspdmdt/AddPoszukiwaniaKartoteka', JSON.stringify({identifier: identifier, reasonarea: reasonarea}), function(poszukiwaniadata){
                $('.moreinfo-poszukiwania').append(
                    `<div class="file-content-holder" style="margin-top: 0.5%;">
    ...
    $('#file-button-notes').on('click', function(){
        ...
        if (note != "") {
            ...
            $.post('http://esx_lspdmdt/AddNotatkaKartoteka', JSON.stringify({identifier: identifier, note: note}), function(notedata){
                $('.moreinfo-notatki').append(
                    `<div class="file-content-holder" style="margin-top: 0.5%;">
...
$(document).ready(function(){
    ...
    for (var i = 0; i < test.length; i++) {    
        test[i].onclick = function() {
            ...
            qee.innerHTML = "Nie";
            $.post('http://esx_lspdmdt/licencjaUsun', JSON.stringify({identifier: identifier, type: type}));
        }
    }
    ...
    for (var i = 0; i < test2.length; i++) {    
        test2[i].onclick = function() {
            ...
            var identifier = $("#file-personnametype").data("identifier");
            qee.innerHTML = "Tak";
            $.post('http://esx_lspdmdt/licencjaDodaj', JSON.stringify({identifier: identifier, type: type}));
        }
    }
});
