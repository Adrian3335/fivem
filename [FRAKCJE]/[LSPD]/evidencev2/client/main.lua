...
exports['qtarget']:AddBoxZone("laboratoriumpd", vector3(483.85, -987.65, 30.69), 1, 1, {
	name="laboratoriumpd",
	heading=0,
	--debugPoly=false,
	minZ=30.66,
	maxZ=31.99,
		...
		options = {
			{
				event = "analizujdowody",
				icon = "fas fa-fingerprint",
				label = "Analizuj Dowody",
                job = "police",
            ...
            {
				event = "przeczytajraport",
				icon = "fas fa-fingerprint",
				label = "Przeczytaj Raport",
                job = "police",
                canInteract = function()
                if not analyzing and analyzingDone then
                    return true end
                end,
			},
		},
...
RegisterNetEvent('analizujdowody', function()
    if not analyzing and not analyzingDone then
        if #evidence > 0 then
            Citizen.CreateThread(
                function()
                    ...
                    analyzingDone = true
                    analyzing = false
                    ESX.ShowNotification('Dowody Zostały przeanalizowane')
                end
            )
        else
            SendTextMessage(Config.Text["no_evidence_to_analyze"])
...
RegisterNetEvent('przeczytajraport', function()
if not analyzing and analyzingDone then
    SendNUIMessage(
        {
            type = "showReport",
...
exports['qtarget']:AddBoxZone("archiwumdowodowpd", vector3(446.19, -997.15, 30.69), 3, 1, {
	name="archiwumdowodowpd",
	heading=0,
	--debugPoly=false,
	minZ=30.66,
	maxZ=30.99,
		...
		options = {
			{
				event = "archiwumdowodowe",
				icon = "fas fa-id-card",
				label = "Archiwum dowodów",
                job = "police",
			},
		},
		distance = 3.5
})
RegisterNetEvent('archiwumdowodowe', function()
    ESX.TriggerServerCallback(
        "core_evidence:getStorageData",
        function(data)
            ESX.UI.Menu.CloseAll()
