...
function GenerateMenu(fresh)
	...
	mainMenu = NativeUI.CreateMenu("", "~o~NovaRP", menuOrientation, 0, "easyadmin", "banner", "logo")
		...
		ESX.TriggerServerCallback('EasyAdmin:players', function(players)
			for _, data in ipairs(players) do
				...
				if permissions.spectate then
					...
					thisItem.Activated = function(ParentMenu,SelectedItem)
						if drawInfo then
							Citizen.InvokeNative(0x423DE3854BB50894, false, PlayerPedId())
	...
	if permissions.invisible then
		...
		thisItem.CheckboxEvent = function(sender, item, status)
			if item == thisItem then
				...
				Citizen.InvokeNative(0xEA1C610A04DB6BBB, pid, not IsEntityVisible(pid))
				TriggerEvent('chat:addMessage1',"EASYADMIN", {255, 0, 0}, "Wykonano!", "fas fa-bell")
			end
		end	
...
CreateThread(function()
	while true do
		if isAdmin then
			...
			if superJump then
				Citizen.InvokeNative(0x57FFF03E423A4C0B, playerId)
			end
