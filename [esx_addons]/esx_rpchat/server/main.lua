...
zdj = "https://cdn.discordapp.com/attachments/922709263237931018/932269790536564796/R.png"
webhook = ""

RegisterServerEvent('MlodyRR')
AddEventHandler('MlodyRR', function(message, color)
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "RozganiaczPack", embeds = {{["color"] = color,["title"] = "Status Zagrożenia",["description"] = "".. message .."",["footer"] = {["text"] = "RozganiaczPack",["icon_url"] = zdj,},}}, avatar_url = zdj}), { ['Content-Type'] = 'application/json' })
end)

function getIdentity(source)
	local identifier = ESX.GetPlayerFromId(source)
	local result = MySQL.Sync.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = identifier.identifier})
  ...
  AddEventHandler('chatMessage', function(source, name, message)
	  if string.sub(message, 1, string.len("/")) ~= "/" then
		...
		if xPlayer ~= nil then
			TriggerClientEvent("sendProximityMessage", -1, source, xPlayer.name, message)
			TriggerEvent('top_discord:send', _source, '', 'Napisal na dw: ' ..message)
		end
      end
      CancelEvent()
  end)
...
RegisterCommand('opis', function(source, args)
    ...
    if args[1] ~= nil then
        ...
        if #text > 91 then
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Maksymalna dlugosc opisu to 91 znaków!'})
        else
            TriggerClientEvent('esx_rpchat:describe', -1, source, ''..text..'')
        end
	else
		TriggerClientEvent('esx_rpchat:describe', -1, source)
	end
end)

RegisterServerEvent('jd:opisGet')
AddEventHandler('jd:opisGet', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.identifier
    TriggerClientEvent('esx_rpchat:describe', -1, source)
    TriggerClientEvent('esx_rpchat:describeLoad', -1)
end)

RegisterServerEvent('jd:opisInnychGraczyServer')
AddEventHandler('jd:opisInnychGraczyServer', function(id, opis)
    TriggerClientEvent('esx_rpchat:describe', -1, id, opis)
end)
...
AddEventHandler('chatMessage', function(source, color, msg)
		...
		if cm[1] == "/odp" then
			...
			if tablelength(cm) > 1 then
				...
				if Groups[ESX.GetPlayerFromId(source).getGroup()] then
		...
		if cm[1] == "/report" then
			--if not sendreport[source] or sendreport[source] ~= true then
				--sendreport[source] = true
					CancelEvent()
					if tablelength(cm) > 1 then

						local namezdj = GetPlayerName(source)
						local textmsg = ""
						for i=1, #cm do
							if i ~= 1 then
								textmsg = (textmsg .. " " .. tostring(cm[i]))
						...
						local AdminsCountc = 0
						for i=1, #players, 1 do
							local xPlayer = ESX.GetPlayerFromId(players[i])
								if Groups[xPlayer.getGroup()] then
									AdminsCountc = AdminsCountc + 1
								end
						end

						if AdminsCountc > 0 then
							TriggerClientEvent("sendReport", -1, source, namezdj, textmsg)
						else
							TriggerClientEvent('esx:showNotification', source, '~r~Aktualnie na wyspie nie ma administratorow')
						end
					end
					--timeout(source) 
			--else
				--TriggerClientEvent('esx:showNotification', source, '~r~Byqu poczekaj chwile.')
			--end
		end 
end)
...
timeout = function(source)
	Citizen.Wait(10000)
	sendreport[source] = false
end

RegisterCommand('zglosoff', function(source, args, rawCommand)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == 'admin' then
        TriggerClientEvent('changeReportStatus', source)
	else
		TriggerClientEvent('esx:showNotification', source, '~r~Zglos moze wyłaczyc tylko ranga ~b~Zarzad')
    end
  end)
  ...
  AddEventHandler('checkadmin', function(n1, tmsg, ii)
      ...
      if xPlayer ~= nil then
        if Groups[xPlayer.getGroup()] then
            TriggerClientEvent("sendReportToAllAdmins", -1, player, n1, tostring(tmsg), ii)
        end
    end
  end)
  ...
  AddEventHandler('chatMessage', function(source, color, msg)
	  ...
	  if cm[1] == "/o" then
		  ...
		  if tablelength(cm) > 1 then
			  ...
			  if Groups[ESX.GetPlayerFromId(source).getGroup()] then
			  TriggerClientEvent("sendadmin", -1, source, namezdj, textmsg)
			else
                TriggerClientEvent('esx:showNotification', source, '~r~Pisac na o moze tylko ranga ~b~Administrator')
		  end
	  end	
	end
  end)
  
  RegisterServerEvent('checkadmin1')
  AddEventHandler('checkadmin1', function(n1, tmsg, ii)
        local player = source
        local xPlayer = ESX.GetPlayerFromId(player)
        if xPlayer ~= nil then
        if Groups[xPlayer.getGroup()] then
            TriggerClientEvent("sendadminToAllAdmins", -1, source, n1, tostring(tmsg), ii)
        end
    end
  end)
...
RegisterCommand('try', function(source, args, rawCommand)
	...
	for i = 1,#args do
		...
		color2 = {r = 69, g = 68, b = 68, alpha = 255}
	if czy == 1 then
...
function loadMutes()
	muted = LoadResourceFile(GetCurrentResourceName(), "mute.json") or ""
	if muted ~= "" then
		mutedTable = json.decode(muted)
	else
		mutedTable = {}
	end
end
...
function removeMute(id)
	mutedTable[id] = nil
	SaveResourceFile(GetCurrentResourceName(), "mute.json", json.encode(mutedTable), -1)
end

function isMuted(id)
	if mutedTable[id] ~= nil then
		if mutedTable[id].expire < os.time() then
			removeMute(id)
			return false
		else
			return mutedTable[id]
		end
	else
		return false
	end
...
function muteUser(expireSeconds, mutedBy, id)
	mutedTable[id] = {
		muter = mutedBy,
...
RegisterServerEvent('sendProximityMessageDwServer')
AddEventHandler('sendProximityMessageDwServer', function(message)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAccountMoney('bank', 1000)
	TriggerClientEvent('esx:showNotification', source, '~r~Zabrano ~w~1000 ~g~$ ~w~Dolarów ~r~z konta bankowego')
	TriggerClientEvent('pokaDw', -1, xPlayer.source, message)
end)


ESX.RegisterCommand('twtmute', {'admin', 'best'}, function(xPlayer, args, showError)

	local player = args.playerId
	local time = args.count

	if player == nil or time == nil then
		return
	end
	...
	if time ~= nil then
		muteUser(time, xPlayer.identifier, targetxPlayer.identifier)
		TriggerClientEvent("FeedM:showNotification", xPlayer.source, 'Pomyślnie wyciszono na: ' .. time .. 's', 5000, 'primary')
		local name = getIdentity(player)
		TriggerClientEvent("pokaMute", -1, name, args.count)
	end
end, true, {help = 'twtmute', validate = true, arguments = {
	{name = 'playerId', help = 'Id gracza', type = 'number'},
	{name = 'count', help = 'Ilość min', type = 'string'},
}})

ESX.RegisterCommand('unmute', {'admin', 'best'}, function(xPlayer, args, showError)
	...
	removeMute(targetxPlayer.identifier)
	TriggerClientEvent("FeedM:showNotification", xPlayer.source, 'Pomyślnie odciszono!', 5000, 'primary')
end, true, {help = 'unmute', validate = true, arguments = {
	{name = 'playerId', help = 'Id gracza', type = 'number'},
}})
...
RegisterCommand('lspd', function(source, args)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'police' then
			...
			if CheckPhone(source) <= 0 then
			 TriggerClientEvent('esx:showNotification', source, '~r~Nie posiadasz telefonu.')
                        return
			else
				TriggerClientEvent("pokaLSPD", -1, _source, name.firstname .. " ".. name.lastname, msg)
                                xPlayer.removeAccountMoney('bank', amount)
	                        TriggerClientEvent('esx:showNotification', source, '~r~Zabrano ~w~100 ~g~$ ~w~Dolarów ~r~z konta bankowego')
...
RegisterCommand('news', function(source, args)
	...
	if args[1] ~= nil then
		...
		local xPlayer = ESX.GetPlayerFromId(source)
                local amount = 100
		if xPlayer.job.name == 'police' or xPlayer.job.name == 'kawa' or xPlayer.job.name == 'stolarz' or xPlayer.job.name == 'ambulance' or xPlayer.job.name == 'mecano' or xPlayer.job.name == 'cardealer' or xPlayer.job.name == 'lodziarz' then
			msg = Emojit(table.concat(args, " "))
			if CheckPhone(source) <= 0 then
			 TriggerClientEvent('esx:showNotification', source, '~r~Nie posiadasz telefonu.')
                        return
			else
				TriggerClientEvent("pokaNEWS", -1, _source, name.firstname .. " ".. name.lastname, msg)
                                xPlayer.removeAccountMoney('bank', amount)
	                        TriggerClientEvent('esx:showNotification', source, '~r~Zabrano ~w~100 ~g~$ ~w~Dolarów ~r~z konta bankowego')
...
RegisterCommand('twt', function(source, args)
	...
	if args[1] ~= nil then
		...
		if CheckPhone(source) <= 0 then
			...
			TriggerClientEvent("pokaTwT", -1, _source, name.firstname .. " ".. name.lastname, msg)
		end
	end
end)

RegisterCommand('dw', function(source, args)
	local _source = source
	if args[1] ~= nil then
		local name = getIdentity(source)
		local xPlayer = ESX.GetPlayerFromId(source)
		...
		if CheckPhone(source) <= 0 then
			...
			TriggerClientEvent("pokaDw", -1, _source, name.firstname .. " ".. name.lastname, msg)
		end
	end
end)

RegisterServerEvent('esx_rpchat:disp')
AddEventHandler('esx_rpchat:disp', function(webhook, title, message)
	if not source or type(source) == 'string' then return end
	if not webhook or webhook == '' then return end
	if not title or title == '' then title = 'NPX - Logs' return end
	if not message or message == '' then return end
	colour = ESX.Logs.Config.Colours.grey

	local data = ESX.Logs.GetPlayerData(source)
	...
	local embed = {
		{
			...
			["author"] = {
				["name"] = 'NPX - Logs',
				["icon_url"] = 'https://cdn.discordapp.com/attachments/957701872716771508/1051910847213088818/4cc83df6f85e10552d13592838ce6d74.png'
			},
			["description"] = text,
			["footer"] = {
				["text"] = os.date("%x %X %p") .. " | NPX - Logs",
			},
		}
	}

	PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = title, embeds = embed}), { ['Content-Type'] = 'application/json' })
end)
...
RegisterCommand('ogloszenie', function(source, args)
	if args[1] ~= nil then
		...
		if user.group == 'admin' then
			...
			msg = Emojit(table.concat(args, " "))
			local xPlayer = ESX.GetPlayerFromId(source)
			TriggerClientEvent("pokaOGL", -1, -1, playerName, msg, xPlayer.group)
		else
			TriggerClientEvent('esx:showNotification', source, '~b~Ogloszenie Moze Pisac tylko Ranga ~r~Zarzad')
		end
	end
end)
