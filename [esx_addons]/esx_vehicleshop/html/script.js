    ...
    if (event.data.type == "updateVehicleInfos") {
        ...
        $('#contentVehicle').append(`
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Sterowność</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Max Prędkość</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Przyśpieszenie</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Hamowanie</span>
                        <div class="bar">
                    ...
                    <div class="row" id="actionButton">
                        <a class="buyButton" style=" color:#FC1259;" onclick="openModalMenu()"> Zakup </a>
                    </div>
                `);

                if (isEnableTestDrive) {
                    $('#actionButton').append(`
                        <a class="tdButtton" onclick="testDrive()"> Jazda Testowa </a>
                    `);
                }
    }
...
function menuVehicle(event) {
    ...
    $.post('https://esx_vehicleshop/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://esx_vehicleshop/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function openModalMenu(){    
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-content">
            <p class="title">Potwierdzenie Zakupu:</p>
            <p class="vehicle">Pojazd</p>         

            <p>Marka: <span class="brand">`+vehicleSelected.brand+`</span></p>
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://esx_vehicleshop/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('https://esx_vehicleshop/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://esx_vehicleshop/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://esx_vehicleshop/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://esx_vehicleshop/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://esx_vehicleshop/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://esx_vehicleshop/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://esx_vehicleshop/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
