$(document).ready(function(){
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://esx_garages/close', JSON.stringify({}));
		setTimeout(function(){
			if ($('#btnCar1').length > 0) {
  ...
  document.addEventListener('click',function(e){
	for (var i=1; i<=number; i++) {
		if(e.target && e.target.id == 'btnCar' + i){
			...
			$.post('http://esx_garages/pullCar',  JSON.stringify({ model: $("#btnCar" + i).attr('model') }));
			setTimeout(function(){
				for (var j=1; j<=number; j++) {
					document.getElementById("btnCar" + j).remove();
				}
  ...
  document.addEventListener('click',function(e){
	for (var i=1; i<=number2; i++) {
		if(e.target && e.target.id == 'impCar' + i){
			...
			$.post('http://esx_garages/towCar',  JSON.stringify({ model: $("#impCar" + i).attr('model') }));
			setTimeout(function(){
				for (var j=1; j<=number2; j++) {
  ...
  document.addEventListener('click',function(e){
	for (var i=1; i<=number3; i++) {
		if(e.target && e.target.id == 'policeCar' + i){
			...
			$.post('http://esx_garages/impoundCar',  JSON.stringify({ model: $("#policeCar" + i).attr('model') }));
			setTimeout(function(){
				for (var j=1; j<=number3; j++) {
