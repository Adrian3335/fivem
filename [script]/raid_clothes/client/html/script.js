...
$(function () {
    ...
    window.addEventListener('message', function (event) {
        if (event.data.type == "enableclothesmenu") {
            ...
            $("#button-clothes").css("display", "none");
            open = event.data.enable;
            newPlayer = event.data.newPlayer;
            skinMenu = event.data.skinMenu;
            if(skinMenu) {
                setTimeout(function(){
                    $(".ciuchy").css("display", "block");
                    $(".pedbt").css("display", "block");
                    $(".model").css("display", "block");
                }, 1)
                $('.button-menu').removeClass('actived')
                $('.model').addClass('actived')
                $("#button-ped").css("display", "block");
                model()
            }
            else if(event.data.menu == 'clothesmenu') {
                $('.button-menu').removeClass('actived')
                $('.ciuchy').addClass('actived')
                clothes()
                $(".ciuchy").css("display", "block");
                $(".pedbt").css("display", "none");
                $(".model").css("display", "block");
                $("#button-clothes").css("display", "block");
                $("#button-access").css("display", "block");
                $("#button-ped").css("display", "none");
            }
            else if(event.data.menu == 'barbermenu'){
                barber()
                $('.pedbt').addClass('actived')
            }
            else{
                $(".ciuchy").css("display", "none");
                $(".pedbt").css("display", "none");
    ...
    document.onkeyup = function (data) {
        if (open) {
            ...
            } else if(data.which == 192) {
                $.post('http://raid_clothes/handsup', JSON.stringify({}));
            } else if(data.which == 88) {
                $.post('http://raid_clothes/stopanim', JSON.stringify({}));
            }
        }
    };
    ...
    $('.model').on('click', function() {
        ...
        $("#barbermenu").css("display", "none")
        $("#clothesmenu").css("display", "block")
        $("#button-ped").css("display", "block");
        $("#button-clothes").css("display", "none");
        $("#button-access").css("display", "none");
        model()
    ...
    $('.pedbt').on('click', function() {
        currentMenu = $('#barbermenu');
        $("#clothesmenu").css("display", "none")
        $("#barbermenu").css("display", "block")
        $("#button-ped").css("display", "none");
        $("#button-clothes").css("display", "none");
        $("#button-access").css("display", "none");
        barber()
    ...
    function CloseMenu(save, barber, noclose) {
        if(newPlayer == undefined) {
            newPlayer = false
        }
        $.post('http://raid_clothes/escape', JSON.stringify({save:save, newPlayer:newPlayer, skinMenu:skinMenu, barber: barber, noclose: noclose}));
        //newPlayer   = false
        skinMenu    = false
    }
    ...
    function UpdateInputs(drawables, props, drawtextures, proptextures, sex) {
    ...
    $('.tog_hat').on('click', function() {
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "helmet"}))
    })
    ...
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "torso"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "tshirts"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "torso"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "bproof"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "chain"}))
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "pants"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://raid_clothes/toggleclothes', JSON.stringify({name: "mask"}))
    })
